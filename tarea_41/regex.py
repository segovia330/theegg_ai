...
# path completo del fichero con el texto
nombre_file = input()

f = open (nombre_file,'r')
cadena = f.read()
    # print(cadena)

#cadena = 'En Strandhill, Irlanda, se cruzó en mi camino Chris, un señor de los que inspiran y se posicionan como \
...
lista_letras = patron_letra.findall(cadena)
lista_palabras = patron_palabra.findall(cadena)


# letras
print('Número de letras', len(lista_letras))  # sin contrar espacios.
# print(lista_letras)


# palabras
print('Número de palabras', len(lista_palabras))   # 'Número de palabras ', 
# print(lista_palabras)



# frecuencias de palabras
# quitar palabras repetidas en la lista
lista_palabras_norep = []
for palabra in lista_palabras:
    if palabra not in lista_palabras_norep:
        lista_palabras_norep.append(palabra)

# añadir en otra lista cada palabra y su número de apariciones. 
lista_palabras_frec = []
for palabra in lista_palabras_norep:
    # buscamos el número de apariciones de la palabra
    patron_una_palabra = re.compile(r'\b' + palabra + r'\b')
    lista_esa_palabra = patron_una_palabra.findall(cadena)
    # print(palabra, len(lista_esa_palabra))
    lista_palabras_frec.append(tuple([palabra, len(lista_esa_palabra)]))


# ordenamos lista por numero de apariciones
lista_palabras_frec_ord = sorted(lista_palabras_frec, key=lambda tup: tup[1], reverse=True)

# printamos palabras por frecuencia
print('Ranking por palabras:')
for tupla in lista_palabras_frec_ord:    
    print('{:<20s}{:>4d}'.format(tupla[0], tupla[1]))

# print(lista_palabras_frec_ord)
